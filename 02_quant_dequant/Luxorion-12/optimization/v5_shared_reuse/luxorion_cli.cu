...
__global__ void nv_block(const T *x, std::uint8_t *data, std::uint8_t *scales,
  ...
  __syncthreads();
  c[lane] = i < n ? encode_e2(v / scale, sr, i, seed) : 0;
  __syncthreads();
  if ((lane & 1U) == 0)
...
__global__ void mx_deq(const std::uint8_t *data, const std::uint8_t *scales,
  ...
  __shared__ float cached_scales[kDequantThreads / 8];
  constexpr unsigned kScalesPerCudaBlock = kDequantThreads / 8;
  if (tensor) {
    if (threadIdx.x == 0)
      cached_scales[0] = e8(scales[0]);
  } else if (threadIdx.x < kScalesPerCudaBlock) {
    const std::size_t scale_index =
        static_cast<std::size_t>(blockIdx.x) * kScalesPerCudaBlock +
        threadIdx.x;
    const std::size_t scale_count = (n + 31) / 32;
    cached_scales[threadIdx.x] =
        scale_index < scale_count ? e8(scales[scale_index]) : 0.0F;
  }
  __syncthreads();
  ...
  const float scale = cached_scales[tensor ? 0 : threadIdx.x / 8];
  if (i + 4 <= n) {
...
__global__ void nv_deq(const std::uint8_t *data, const std::uint8_t *scales,
  ...
  if (tensor) {
    if (threadIdx.x == 0)
      cached_scales[0] = e4(scales[0]);
  } else if (threadIdx.x < kScalesPerCudaBlock) {
    const std::size_t scale_index =
        static_cast<std::size_t>(blockIdx.x) * kScalesPerCudaBlock +
        threadIdx.x;
    const std::size_t scale_count = (n + 15) / 16;
    cached_scales[threadIdx.x] =
        scale_index < scale_count ? e4(scales[scale_index]) : 0.0F;
  }
  __syncthreads();
  ...
  const float local_scale = cached_scales[tensor ? 0 : threadIdx.x / 8];
  const float a =
