...
__global__ void nv_deq(const std::uint8_t *data, const std::uint8_t *scales,
  ...
  if (i < n) {
    auto p = data[i / 2];
    auto c = static_cast<std::uint8_t>((i & 1U) ? p & 15U : p >> 4);
    store(out, i, e2(c) * global * e4(scales[tensor ? 0 : i / 16]));
  }
}
...
Dequantized dequant_impl(const Quantized &q, unsigned w, unsigned r) {
  ...
  auto launch = [&] {
    if (q.format == QuantFormat::Mxfp8)
      mx_deq<<<blocks, 256>>>(dd.get(), ds.get(), out.get(), n, tensor);
    else
      nv_deq<<<blocks, 256>>>(dd.get(), ds.get(), q.global_scale, out.get(), n,
                              tensor);
...
int main(int argc, char **argv) {
  try {
    ...
    if (op == "run") {
      ...
      std::cout << "restored: " << fs::absolute(argv[3]) << '\n';
    } else
