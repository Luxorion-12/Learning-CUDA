...
__device__ std::uint8_t encode_e4(float value, bool stochastic,
  ...
  for (unsigned c = 1; c <= 0x7EU; ++c) {
    double v = e4(static_cast<std::uint8_t>(c));
    if (v <= x)
      lower = c;
    if (v >= x) {
      upper = c;
...
template <typename T>
__device__ float warp_max(float value, unsigned mask, int width = 32) {
  for (int offset = width / 2; offset > 0; offset >>= 1)
    value = fmaxf(value, __shfl_down_sync(mask, value, offset, width));
  return value;
}

template <typename T>
__global__ void amax_partial(const T *x, std::size_t n, float *partial) {
  __shared__ float warp_maxima[kReduceThreads / 32];
  float v = 0;
  for (std::size_t i =
  ...
  v = warp_max<T>(v, __activemask());
  if (lane == 0)
    warp_maxima[warp] = v;
  __syncthreads();
  if (warp == 0) {
    v = lane < kReduceThreads / 32 ? warp_maxima[lane] : 0.0F;
    v = warp_max<T>(v, 0xffffffffU);
    if (lane == 0)
...
__global__ void finish_amax(const float *p, unsigned n, float *out) {
  ...
  const unsigned lane = threadIdx.x & 31U;
  const unsigned warp = threadIdx.x >> 5U;
  v = warp_max<float>(v, __activemask());
  if (lane == 0)
    warp_maxima[warp] = v;
  __syncthreads();
  if (warp == 0) {
    v = lane < kReduceThreads / 32 ? warp_maxima[lane] : 0.0F;
    v = warp_max<float>(v, 0xffffffffU);
    if (lane == 0)
      *out = v;
  }
}
...
__global__ void mx_block(const T *x, std::uint8_t *data, std::uint8_t *scales,
  ...
  const float amax = warp_max<T>(fabsf(v), mask);
  std::uint8_t c = 0;
  if (lane == 0) {
    c = select_e8(amax);
    scales[blockIdx.x] = c;
  }
  c = static_cast<std::uint8_t>(__shfl_sync(mask, static_cast<unsigned>(c), 0));
  const float scale = e8(c);
  if (i < n)
...
__global__ void nv_block(const T *x, std::uint8_t *data, std::uint8_t *scales,
  ...
  const float amax = warp_max<T>(fabsf(v), mask, 16);
  const float global_scale =
      __shfl_sync(mask, lane == 0 ? *global : 0.0F, 0, 16);
  std::uint8_t scale_code = 0;
  if (lane == 0) {
    float ideal = amax == 0 ? 1.0F : (amax / 6.0F) / global_scale;
    scale_code = encode_e4(ideal, false, blockIdx.x, seed);
    if (amax != 0 && scale_code == 0)
      scale_code = 1;
    scales[blockIdx.x] = scale_code;
  }
  scale_code = static_cast<std::uint8_t>(
      __shfl_sync(mask, static_cast<unsigned>(scale_code), 0, 16));
  const float scale = global_scale * e4(scale_code);
  const std::uint8_t code =
      i < n ? encode_e2(v / scale, sr, i, seed) : 0;
  const auto next = static_cast<std::uint8_t>(
      __shfl_down_sync(mask, static_cast<unsigned>(code), 1, 16));
  if ((lane & 1U) == 0)
    data[static_cast<std::size_t>(blockIdx.x) * 8 + lane / 2] =
        static_cast<std::uint8_t>((code << 4) | next);
}
template <typename T>
